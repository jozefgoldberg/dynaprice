function dpp_request(action,urlRef) {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && (xhr.status == 200 || xhr.status == 0)) {
            dppReadResponse(action,xhr.responseXML); 
        }
    };
    var httpAdr ="";
    if(action == 'access') {httpAdr = dppHttpAdr + 'access/' + dppCliDomaine  + '/' + dppUid};
    if(action == 'achat') {httpAdr = dppHttpAdr + 'achat/' + dppCliDomaine  + '/' + dppUid};
    if(action == 'product') {
    ...
    if(action == 'category') {
        ...
        httpAdr = dppHttpAdr +  'category/' + dppCliDomaine  + '/' + dppUid + '/' +urlRef;
    }
